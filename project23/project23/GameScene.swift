...
//  project23
//
//  Created by Anisha Lamichhane on 9/8/20.
//
...
//                destroy the bomb
                guard let container = node.parent as? SKSpriteNode else { continue }
                ...
                if let index = activeEnemies.firstIndex(of: container) {
                    activeEnemies.remove(at: index)
                }
    ...
    func playSooshSound() {
        isSooshSoundActive = true
        
        let randomNumber = Int.random(in: 1...3)
        let soundName = "swoosh\(randomNumber).caf"
        let swooshSound = SKAction.playSoundFileNamed(soundName, waitForCompletion: true)
        run(swooshSound){ [weak self] in
            self?.isSooshSoundActive = false
        }
    }
    ...
    func redrawActiveSlice() {
        if activeSlicePoints.count == 0 {
            activeSliceBG.path = nil
            activeSliceFG.path = nil
            return
        } else if activeSlicePoints.count > 12 {
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if bombCount == 0 {
//            we need to stop the fuse sound
            bombSoundEffect?.stop()
            bombSoundEffect = nil
        }
    }
